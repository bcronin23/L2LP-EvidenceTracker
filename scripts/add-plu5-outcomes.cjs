...
const outcomes = JSON.parse(fs.readFileSync(seedPath, 'utf8'));

// New outcomes 5.24-5.32 for JC_L2LP Preparing for Work
const newOutcomes = [
...
if (existing.length > 0) {
  console.log('Outcomes 5.24+ already exist, skipping');
  process.exit(0);
}
...
console.log(`Added ${newOutcomes.length} new outcomes to Preparing for Work`);
console.log('New total Preparing for Work:', outcomes.filter(o => o.programme === 'JC_L2LP' && o.plu_or_module_code === 'JC-L2-PLU5').length);
